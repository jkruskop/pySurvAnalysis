...
| `pysurvanalysis/apps/_hub_tiles.py` | `pytrackinganalysis/apps/_hub_tiles.py` | was verbatim (`Ptrack*` objectNames → `Psurv*`); now + the full dim treatment (background/title/icon), `TilePanel._content_height`, and `TilePanel.cards()` so the Hub can dim a whole panel. `StatusTile.mousePressEvent` now **accepts** a left press instead of passing it to QFrame: an ignored press is re-delivered to every ancestor, and after a sub-tile has swapped panels the Hub's click-away filter judged that late delivery against the new, narrower panel and closed it at once (the Scripts and AI sub-tiles, beyond its width, never showed one). `ClickAwayFilter` also hands the Hub the press's receiver, and `_handle_click_away` hit-tests in window coordinates rather than `QApplication.widgetAt(globalPosition)`, which needs a screen position a Wayland window does not have. Upstream's tile and handler have the same construction. **Worth backporting** |
...
| `pysurvanalysis/report_builder.py` (`_curated_figure`) | `pytrackinganalysis/report_figures.py` | **rule ported, body local** — upstream's report-native figures replace pasted-in analysis PNGs; here the same precedence (curated Publication Figure over the analysis's default PNG, default as the fallback, a failing render never sinking the report) is applied per plot id from the container's `plot_specs.yaml`, rendered through the plotnine path at 200 dpi |
